...
void GameLose::LoseStateInit(gef::Platform& platform,
	gef::InputManager* input_manager_,
	gef::SpriteRenderer* sprite_renderer,
	gef::Font* font)
{
	button_icon_ = CreateTextureFromPNG("playstation-circle-dark-icon.png", platform);
	BG = CreateTextureFromPNG("GameBG.png", platform);
...
int GameLose::LoseStateUpdate(float frame_time) {
	return 8;
}

void GameLose::LoseStateRender() {
	sprite_renderer_->Begin();
	RenderBG();
	RenderCenteredText("GAMEOVER!!!");
	RenderPressButtonPrompt(button_icon_, "To Quit", &buttonXPos, &buttonYPos);
	sprite_renderer_->End();

}

void GameLose::LoseStateRelease() {
	delete button_icon_;
	delete BG;
	button_icon_ = NULL;
}

void GameLose::RenderBG() {
...
void GameLose::RenderCenteredText(char* text)
{
	font_->RenderText(
		sprite_renderer_,
...
void GameLose::RenderPressButtonPrompt(gef::Texture* tex, char* text, float* Xpos, float* Ypos)
{
